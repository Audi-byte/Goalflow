  ...
  return (
    <nav style={{ display: 'flex', borderTop: '1px solid rgba(245,166,35,0.15)', background: '#0F0F0F', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100 }}>
      {tabs.map(({ id, label, Icon }) => (
        <button key={id} onClick={() => setActive(id)} style={{ flex: 1, background: 'none', border: 'none', cursor: 'pointer', padding: '0.6rem 0 0.4rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
          <Icon size={17} color={active === id ? '#F5A623' : '#8A7A6E'} />
          <span style={{ fontSize: '8px', color: active === id ? '#F5A623' : '#8A7A6E', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{label}</span>
        </button>
      ))}
    </nav>
  )
